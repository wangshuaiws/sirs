...
export function useKlineData() {
  ...
  /** 聚合加载，按 ts 去重后合并到 klineData */
  async function loadRange(code: string, from: string, to: string) {
    loading.value = true
    try {
      ...
      // 去重合并
      const existing = new Set(klineData.value.map((d: any) => d.ts))
      const unique = newData.filter((d: any) => !existing.has(d.ts))

      if (unique.length === 0) return 0

      // 按时间排序
      klineData.value = [...unique, ...klineData.value].sort(
        (a: any, b: any) => a.ts.localeCompare(b.ts),
      )
